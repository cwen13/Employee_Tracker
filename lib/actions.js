...
// Done
var viewEmployees =
    "SELECT e.id AS 'Employee ID', e.first_name AS 'First Name,e.last_name AS 'Last Name', role.title AS 'Job Title', department.name AS 'Department', role.salary AS 'Salary', CONCAT(em.first_name,' ',em.last_name) AS 'Their Manager'FROM employee AS e employee AS em LEFT JOIN role ON e.role_id=role.id LEFT JOIN department on role.department_id = department.id LEFT JOIN em ON e.id = em.manager_id;"
